...
import { canManageTeam, requireAnyRole, revokeUserSessions } from "@/lib/auth";
...
export async function inviteTeamUserAction(formData: FormData) {
  const user = await requireAnyRole([UserRole.OWNER, UserRole.MANAGER]);
...
export async function transferOwnershipAction(targetUserId: string) {
  const user = await requireAnyRole([UserRole.OWNER]);
  ...
  await prisma.$transaction([
    ...
    prisma.auditEvent.create({ data: { action: "team.ownership_transferred", firmId: user.firmId, userId: user.id, entityType: "User", entityId: targetUserId } })
  ]);
  revalidatePath("/app/settings/team");
}
