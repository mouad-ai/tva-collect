  ...
  return (
    <div className="content-stack">
      <div>
        ...
        <p className="text-sm text-muted">Regles TVA du cabinet: pays, devise, frequence, échéances, taux et regimes.</p>
      </div>

      <section className="card p-4">
        <div className="mb-4 flex items-center gap-2">
          <Landmark size={18} />
          <h2 className="font-extrabold">Regles TVA par defaut</h2>
        </div>
        <form action={updateFiscalConfigAction} className="grid gap-4">
          <div className="field-grid">
            ...
            <label>Jour declaration<input name="defaultDeclarationDay" type="number" min={1} max={31} defaultValue={config?.defaultDeclarationDay || 20} /></label>
        ...
        <div className="card p-4">
          ...
          <form action={createTvaRateAction} className="grid gap-3">
            <div className="grid gap-3 md:grid-cols-[1fr_120px]">
              <label>Libelle<input name="label" placeholder="TVA normale" required /></label>
              <label>Taux %<input name="rate" type="number" step="0.01" min={0} placeholder="20" required /></label>
            </div>
            <label className="flex items-center gap-2 text-sm font-bold"><input name="isDefault" type="checkbox" /> Taux par defaut</label>
            ...
            {rates.map((rate) => (
              <div key={rate.id} className="flex items-center justify-between gap-3 rounded-md border border-border p-3 text-sm">
                <div>
                  ...
                  <div className="text-muted">{percentValue(rate.rate)}% {rate.isDefault ? "- taux par defaut" : ""}</div>
                </div>
                <span className={rate.isActive ? "font-bold text-emerald-700" : "font-bold text-slate-400"}>{rate.isActive ? "Actif" : "Inactif"}</span>
              </div>
            ))}
            {!rates.length ? <div className="rounded-md border border-amber-200 bg-amber-50 p-3 text-sm font-bold text-amber-900">Aucun taux TVA configure.</div> : null}
          </div>
        </div>

        <div className="card p-4">
          <h2 className="mb-4 font-extrabold">Regimes fiscaux</h2>
          <form action={createFiscalRegimeAction} className="grid gap-3">
            <label>Nom du regime<input name="name" placeholder="TVA mensuelle standard" required /></label>
            <label>
              Frequence
            ...
            <label>Description<textarea name="description" rows={3} placeholder="Regime utilise pour les clients declarant la TVA chaque mois." /></label>
          ...
          <div className="mt-4 grid gap-2">
            ...
            {!regimes.length ? <div className="rounded-md border border-amber-200 bg-amber-50 p-3 text-sm font-bold text-amber-900">Aucun regime fiscal configure.</div> : null}
          </div>
        </div>
      </section>
    </div>
