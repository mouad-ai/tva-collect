...
export async function createFirmWithOwnerAction(formData: FormData) {
  ...
  const trialStartsAt = text(formData, "trialStartsAt") ? new Date(String(text(formData, "trialStartsAt"))) : new Date();
  const trialEndsAt = text(formData, "trialEndsAt")
